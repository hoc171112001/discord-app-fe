...
export const LoginEntries: FC<IProps> = ({ lang }) => {
  ...
  return (
    <div className="form-inputs">
      <div className="my-4">
        <label htmlFor="email" className="text-desc font-semibold mb-2 text-xs label-required inline-block">
          {t('emailOrPassword')}
        </label>
        <TextField size="small" id="email" type="email" fullWidth />
      </div>
      <div className="mt-4 mb-2">
        <label htmlFor="password" className="text-desc font-semibold mb-2 text-xs label-required inline-block">
          {t('password')}
        </label>
        <TextField size="small" id="password" type="password" fullWidth />
      </div>
      <span className="text-sm text-blue-500 hover:cursor-pointer hover:underline">{t('forgotPw')}</span>
      <Button style={{ margin: '20px 0 5px 0' }} fullWidth size="large" variant="contained">
        {t('submitBtn')}
      </Button>
      <p className="text-s">
